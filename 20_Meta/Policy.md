...
- **Ephemeral Workspace Policy (임시 작업장 클리닝 정책):**
  - `temp/` 폴더 내에 생성된 일회성 파이썬 스크립트 등 임시 도구들은 1회 사용 직후 더 이상 필요하지 않을 경우 즉시 자동 삭제(비워주기) 처리하여 불필요한 파일이 쌓이지 않도록 쾌적한 환경을 유지합니다.
