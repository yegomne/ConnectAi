...
- **작업 지시어 기반 자동 파이프라인 (Trigger Protocol):**
  - **[암묵적 자동 실행 (Implicit Execution)]** 대표님이 "지식화", "구조화"라는 단어를 명시적으로 쓰지 않고 단지 데이터(링크, 텍스트, 원본 파일)만 전달하거나, "작업해", "정리해", "이거 봐" 등의 짧은 지시만 내리더라도, 묻지도 따지지도 않고 디폴트로 즉시 `📝 지식 문서 변환 규격(The Wiki Template)`을 적용하여 7단계 지식화 구조화 파이프라인을 강제 실행해야 합니다. 가만히 있거나 되묻지 마십시오. 최종 보고는 반드시 A-Wiki 표준 답변 포맷과 마크다운 코드 블록으로 출력해야 합니다.
