...
> 현재 단계: **3단계 GUI 완성** (메인 + 이력 + 통계 + 엑셀) → 4단계 (NSFW 스크리너 + 누끼) 진입 직전
...
│   ├── components.py           # 카드 컴포넌트 (페이즈 2, 토글은 3.5)
│   ├── excel_export.py         # 2시트 엑셀 출력 (페이즈 3)
│   └── pages/
│       ├── 1_📜_이력.py        # 이력 페이지 (페이즈 3)
│       └── 2_📊_키워드_통계.py # 키워드 통계 (페이즈 3)
├── data/
...
- **gui/pages/1_📜_이력.py**: 처리 이력 페이지
  - 상단 테이블 + 행 클릭 시 하단 카드 상세 (혼합 표시)
  - 검색(키워드), 카테고리 필터, 기간 필터
  - 재선택: 이전 처리의 5개 후보 중 다른 거 선택 → 새 이력으로 INSERT (이력 보존)
  - 키워드 차단: 직접조립 이력의 사용 키워드를 카테고리에서 차단
  - 엑셀 다운로드 버튼
- **gui/pages/2_📊_키워드_통계.py**: 강화학습 데이터 시각화 (보너스 B1)
  - 탭 ① 카테고리별 TOP (막대 차트 + 표)
  - 탭 ② 전체 통합 TOP 30
  - 탭 ③ 차단된 키워드 관리 (차단 해제)
- **gui/excel_export.py**: 2시트 분리 출력
  - 시트 "업로드용": 처리시각/원본/선택상품명/글자수 (마켓 복붙용)
  - 시트 "분석용": 카테고리/후보번호/응답시간/사용키워드/모델 (통계용)
  - 파일명 `result_YYYYMMDD_HHMMSS.xlsx`, 한글 폭 고려 컬럼 너비 자동
- **storage.py 추가 함수**: get_all_keyword_stats, get_overall_top_keywords, get_blocked_keywords (통계 페이지용 조회, SQL은 storage에만)

#### 페이즈 3.5: 6번째 카드 키워드 토글 ✅
- **문제**: 칩 클릭 시 무조건 추가만 됨 (같은 칩 다시 눌러도 중복 추가)
- **해결**: 순수 토글 방식 (방식 A) — 클릭=추가, 다시 클릭=제거
- keywords 리스트를 SSOT로, 텍스트는 리스트로부터 재생성
- 선택된 칩은 ✓ 표시 + primary 색 강조
...
- **사용자 영향**: 키워드 칩 클릭(토글)은 실시간 반응. 직접 타이핑만 지연.
- **안전망**: 50자 초과 시 결정 버튼 자동 비활성화 (포커스 잃는 순간 검증)

### 3단계 완성 — 실사용 검증 대기
GUI 전체 완성. 다음은 1주일 실사용으로 엣지 케이스 수집 + 강화학습 데이터 누적.
단, 사장님 결정으로 실사용과 4단계를 병행 (문서화 후 바로 4단계 진입).
...
### 3단계 GUI ✅ 완성
메인 화면, 이력 페이지, 키워드 통계, 엑셀 출력 모두 완성.

### 4단계: NSFW 스크리너 + 누끼 추출 ← **다음 작업**
...
21. **글자수 제한: 50자 고정**
22. **저장 시 키워드 순서 기록**
23. **6번째 카드 키워드 칩: 순수 토글** (클릭=추가/다시 클릭=제거, keywords 리스트 SSOT)
...
└── adult_product_renamer\
    ...
    │       ├── 1_📜_이력.py                       ★ 페이즈 3
    │       └── 2_📊_키워드_통계.py                ★ 페이즈 3
    ├── gui\excel_export.py                        ★ 페이즈 3 (2시트 출력)
    └── data\                                     ★ 3단계 신규
        ├── db_schema.sql
        ├── history.db                            (자동 생성)
        └── exports\                              (엑셀 출력 폴더)
...
8. **키워드 토글 SSOT**: 문자열 replace로 키워드 제거 시 부분일치 오류 → keywords 리스트를 단일 진실 소스로 (D-029)

### 다음 단계 시작 전 확인 사항
- [x] 3단계 GUI 완성 (메인 + 이력 + 통계 + 엑셀 + 키워드 토글)
- [ ] **4단계 진입**: NSFW 스크리너 + 누끼 추출
- [ ] 사장님 결정: 비전 모델(llama3.2-vision) 통합 방식 (이미지 전달: 파일 경로 vs Base64)
- [ ] 1주일 실사용 병행 (강화학습 데이터 누적 + 엣지 케이스 수집)
