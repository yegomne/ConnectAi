...
> LLM의 컨텍스트 망각과 과거 지식 편향(60초 쇼츠 룰)으로 인한 에이전트 독단적 행동(환각)을 제어하기 위해, 강제 읽기 프로토콜(Init)과 하드코딩 템플릿 변수화를 도입한 아키텍처 개선안.

## 📖 구조화된 지식 (Synthesized Content)
- **추출된 패턴:** 에이전트 스테이트리스(Stateless) 극복, 프롬프트 파이프라인 강제 변수화, 과거 지식 덮어쓰기.
