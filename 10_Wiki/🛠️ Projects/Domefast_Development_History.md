...
- **최신 마일스톤 및 문제 해결 (2026-05-02):**
  ...
  - **인터랙티브 1:1 정밀 크롭 및 비동기 UX 최적화 (Phase 5-6 완료):**
    - **Byte-Level 직접 크롭:** 화면 캡처 방식의 화질 저하를 막기 위해, 사용자가 마우스로 드래그한 좌표를 원래 이미지 픽셀 좌표계로 환산(`exif_transpose` 보정)하여 원본 Bytes에서 1:1 정사각형으로 직접 자르도록 고도화.
    - **Pending State 자동화 흐름:** 상세 이미지는 즉시 렌더링되지만 백그라운드 Gemini 최적화는 5~10초 걸리는 비동기 구조를 활용. 사용자가 사각형을 그리고 먼저 버튼을 누르면 대기열(Pending)에 저장해두고, AI 완료 즉시 자동으로 병합 처리가 이어지게 하여 대표의 대기 시간을 제로(0)로 만듦.
    - **API 429 에러 UX 개선:** 무료 티어 Gemini API 한도 초과 시 백그라운드에서 조용히 60초 대기하던 현상을 개선. 메인 UI에 `progress_msg` 시그널을 연동하여, 60초 카운트다운 타이머를 실시간으로 띄워 프로그램 중단 여부에 대한 혼란 방지.
...
- **🚀 향후 과제 (To-Do):**
  ...
  - QThread 수명 관리 및 DetailImageWorker stop 플래그 등 파일/스레드 안정화 항목 후속 진행.
